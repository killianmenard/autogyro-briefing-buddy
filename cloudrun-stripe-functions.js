...
// (1) Les 2 price_id du catalogue LIVE (mensuel 4,99 EUR / annuel 49,90 EUR).
const ALLOWED_PRICES = [
  'price_1TtX7J1rgPKfGt9UV4pVzOqT',
  'price_1U1YcW1rgPKfGt9UsGjfSSIf',
];
...
async function getOrCreateCustomer(uid, email) {
  ...
  // Garde-fou : un stripeCustomerId enregistre en mode TEST n'existe pas en
  // mode LIVE (et inversement). Sans cette verification, le premier clic
  // "Passer Pro" apres bascule renvoie "No such customer" -> 500 stripe_error.
  // On verifie que le client existe reellement dans le mode courant ; sinon on
  // en recree un proprement et on ecrase la reference obsolete.
  if (existing) {
    try {
      const found = await stripe.customers.retrieve(existing);
      if (found && !found.deleted) return existing;
    } catch (e) {
      console.warn('stripeCustomerId obsolete, recreation du client', existing);
    }
  }
