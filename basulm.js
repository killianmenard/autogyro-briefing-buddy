...
(async function() {
  ...
  if (mapControls) {
    ...
    legend.innerHTML = `
      <div style="font-size:13px;color:var(--muted-foreground);margin-bottom:4px;">légende des couleurs (${legendItems.length} types)</div>
      <div style="padding-left:2px;">${adDgacRow}${legendHtml}</div>
      <div style="font-size:13px;color:var(--muted-foreground);margin:8px 0 4px;">POI à survoler (4 types)</div>
      <div style="padding-left:2px;">${poiLegendHtml}</div>
    `;
  ...
  function enhanceBasulmCards() {
    ...
    uniquePoints.forEach((ad, i) => {
      ...
      if (bodyEl) {
        ...
        if (b.carb) {
          var _fr = (typeof classifyBasulmFuel === 'function') ? classifyBasulmFuel(b.carb) : null;
          var _ftxt;
          if (_fr && _fr.state === 'on_field') _ftxt = 'sur le terrain' + (_fr.grades.length ? ' : ' + _fr.grades.join(', ') : '');
          else if (_fr && _fr.state === 'on_request') _ftxt = 'sur demande / préavis' + (_fr.grades.length ? ' (' + _fr.grades.join(', ') + ')' : '');
          else if (_fr && _fr.state === 'nearby') _ftxt = 'à proximité (hors terrain) — ' + escapeHtml(b.carb);
          else if (_fr && _fr.state === 'none') _ftxt = 'pas sur place';
          else _ftxt = escapeHtml(b.carb);
          infoHtml += `<div><strong>Carburant :</strong> ${_ftxt}</div>`;
        }
        if (b.pst && b.pst.length) {
        ...
        infoHtml += `<div style="background:#FEF3C7;color:#92400E;padding:6px 8px;border-radius:4px;font-size:11px;margin-top:8px;">
          ℹ️ <strong>Pas de carte VAC officielle</strong> — Cette plateforme BASULM n'est pas publiée par la DGAC. Voir la fiche détaillée sur <a href="https://basulm.ffplum.fr/PDF/${encodeURIComponent(b.c)}.pdf" target="_blank" rel="noreferrer" class="text-blue-600 hover:underline">fiche BASULM (PDF)</a>.
        </div>`;
    ...
    return `
      <div class="card p-4 space-y-4">
        ...
        <div class="text-xs text-muted text-center pt-2">
          AutogyroDash v1.0.11 · <a href="https://github.com/killianmenard/autogyro-briefing-buddy" target="_blank" rel="noreferrer" class="text-blue-600 hover:underline">code source GitHub</a>
        </div>
      </div>
    `;
  }
    ...
    return `
      <div class="card p-4 space-y-5">
        ...
        <div class="text-xs text-muted text-center pt-2 border-t border-thin">
          Version v1.0.11 · <a href="#" id="p-link-sources" class="text-blue-600 hover:underline">voir les sources</a>
        </div>
      </div>
    `;
  }
...
.map-fullscreen .map-fs-close {
  ...
  if (typeof showToast === 'function') {
    showToast(`✓ ${PLATFORMS.length} plateformes ULM · v1.0.11`, 'ok', 3000);
  }
