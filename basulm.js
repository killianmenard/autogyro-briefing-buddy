  ...
  if (mapControls) {
    ...
    try {
      ...
      if (!hasKey) {
        ...
        hint.innerHTML = `<span class="text-muted">Espaces aériens (OpenAIP) non configurés —</span> <a href="#" id="map-openaip-hint-link" class="text-blue-600 hover:underline">configurer dans Paramètres</a>`;
        mapControls.appendChild(hint);
        const link = hint.querySelector('#map-openaip-hint-link');
        if (link) link.addEventListener('click', (e) => {
          e.preventDefault();
          const pTab = document.querySelector('.tab-btn[data-tab="params"]');
          if (pTab) pTab.click();
        });
      }
    } catch (_) {}
    ...
    return `
      <div class="card p-4 space-y-4">
        ...
        <div class="text-xs text-muted text-center pt-2">
          AutogyroDash v0.8.64 · <a href="https://github.com/killianmenard/autogyro-briefing-buddy" target="_blank" rel="noreferrer" class="text-blue-600 hover:underline">code source GitHub</a>
        </div>
      </div>
    `;
  }
    ...
    return `
      <div class="card p-4 space-y-5">
        ...
        <div class="text-xs text-muted text-center pt-2 border-t border-thin">
          Version v0.8.64 · <a href="#" id="p-link-sources" class="text-blue-600 hover:underline">voir les sources</a>
        </div>
      </div>
    `;
  }
...
.map-fullscreen .map-fs-close {
  ...
  if (typeof showToast === 'function') {
    showToast(`✓ ${PLATFORMS.length} plateformes ULM · v0.8.64`, 'ok', 3000);
  }
