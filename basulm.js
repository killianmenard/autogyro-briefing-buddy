...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.24');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.24';
    });
  } catch (e) {}
    ...
    return `
      <div class="card p-4 space-y-4">
        ...
        <div class="text-xs text-muted text-center pt-2">
          AutogyroDash v0.8.24 · <a href="https://github.com/killianmenard/autogyro-briefing-buddy" target="_blank" rel="noreferrer" class="text-blue-600 hover:underline">code source GitHub</a>
        </div>
      </div>
    `;
  }
    ...
    return `
      <div class="card p-4 space-y-5">
        ...
        <div class="text-xs text-muted text-center pt-2 border-t border-thin">
          Version v0.8.24 · <a href="#" id="p-link-sources" class="text-blue-600 hover:underline">voir les sources</a>
        </div>
      </div>
    `;
  }
...
.map-fullscreen .map-fs-close {
  ...
  if (typeof showToast === 'function') {
    showToast(`✓ ${PLATFORMS.length} plateformes ULM · v0.8.24`, 'ok', 3000);
  }
