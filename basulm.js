    ...
    uniquePoints.forEach((ad, i) => {
      ...
      if (bodyEl) {
        const meteoHtml = `<div data-section="metar"><p class="text-sm text-muted">Chargement METAR…</p></div>
          <div data-section="wind" class="mt-3"><p class="text-sm text-muted">Chargement vent…</p></div>`;
        // La grille DGAC pose [METAR/vent] | [carburants] ; côté ULM, on régénère le bloc carburant ici
        // (bodyEl.innerHTML ci-dessous écrase tout .p-4.grid, y compris ce que renderFuelSection() y avait mis à la création)
        const fuelHtml = (typeof renderBasulmFuelBlock === 'function') ? renderBasulmFuelBlock(b) : '';
        bodyEl.innerHTML = `<div>${meteoHtml}</div><div>${fuelHtml}</div>`;
