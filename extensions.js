...
(async function() {
  ...
  function replaceSourcesWithResources() {
    ...
    // Gating Free/Pro : l'onglet Ressources (sigles + TEMSI + espaces + AZBA + sources)
    // est reserve a l'offre Pro. FAIL-OPEN : statut inconnu => contenu complet.
    // Le rendu se re-execute a la resolution du claim via agOnProChange.
    function agResourcesUpsellHtml() {
      return `<div class="card p-4" style="text-align:center;">
        <div style="font-size:26px;margin-bottom:8px;">\u{1F512}</div>
        <h2 class="section-title text-sm" style="margin-bottom:6px;">ressources \u2014 offre Pro</h2>
        <div class="text-xs text-muted" style="max-width:420px;margin:0 auto;line-height:1.5;">
          Le lexique des sigles a\u00e9ronautiques, les symboles TEMSI, le guide des espaces
          a\u00e9riens et les sources d\u00e9taill\u00e9es sont inclus dans l'offre Pro.
        </div>
    ...
    function renderResourcesSection() {
      if (agShouldGate()) {
        resourcesSection.innerHTML = agResourcesUpsellHtml();
        const b = resourcesSection.querySelector('.ag-upsell-btn');
        if (b) b.addEventListener('click', () => { document.getElementById('account-circle')?.click(); });
      } else {
    ...
    if (typeof window.agOnProChange === 'function') {
      window.agOnProChange(() => renderResourcesSection());
    }
