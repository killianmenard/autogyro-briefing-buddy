/* ============================================================
   AutogyroDash — extensions v0.8.68
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.68');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.68';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.68</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.68) — MOBILE « Apple Maps » : carte plein ecran +
   feuille SOLIDE draggable 3 crans + barre recherche = poignee +
   bouton/panneau calques (cran bas) + attribution bas-droite (cran bas).
   Feuille solide (PAS de backdrop-filter) -> sticky fiable sur iOS.
...
@media (max-width: 768px) {
  ...
  /* ---- CALQUES : controle natif masque (sa LISTE est deplacee par JS) ---- */
  ...
  html[data-theme="dark"] #ag-layers-btn { background: #1b2236; }
  #ag-layers-btn svg { width: 24px; height: 24px; display: block; color: #0f1729; }
  html[data-theme="dark"] #ag-layers-btn svg { color: #e6ecff; }
  ...
  html[data-theme="dark"] #ag-layers-panel { background: #1b2236; color: #e6ecff; }
  body.ag-sheet-peek #ag-layers-panel.open { display: block; }
  #ag-layers-panel .leaflet-control-layers-list { display: block !important; }
  #ag-layers-panel label { display: flex; align-items: center; gap: 8px; padding: 7px 2px; font-size: 14px; cursor: pointer; }
  #ag-layers-panel .leaflet-control-layers-separator { border-top: 1px solid rgba(0,0,0,.12); margin: 6px 0; }
  html[data-theme="dark"] #ag-layers-panel .leaflet-control-layers-separator { border-color: rgba(255,255,255,.16); }

  /* ---- FEUILLE SOLIDE draggable (ancree HAUT pour scroll correct) ---- */
  body.ag-mobile-sheet #tab-plan {
    position: fixed; left: 0; right: 0; top: 128px; bottom: auto;
    ...
    box-shadow: 0 -16px 40px rgba(31,42,68,.28);
    transition: transform .28s cubic-bezier(.4,0,.2,1);
    padding: 0 12px calc(86px + env(safe-area-inset-bottom, 0px));
  }
  html[data-theme="dark"] body.ag-mobile-sheet #tab-plan {
    background: #10141e; border-top-color: rgba(255,255,255,.08);
  }

  /* ---- EN-TETE = POIGNEE + RECHERCHE (la barre qu'on tire) ---- */
  #ag-sheet-head {
    position: sticky; top: 0; z-index: 50;
    margin: 0 -12px 12px; padding: 6px 12px 10px;
    background: #f4f6fb;
    border-radius: 22px 22px 0 0;
    box-shadow: 0 8px 10px -8px rgba(31,42,68,.28);
    touch-action: none;
  }
  html[data-theme="dark"] #ag-sheet-head { background: #10141e; }
  #ag-sheet-handle { height: 22px; display: flex; align-items: center; justify-content: center; }
  #ag-sheet-handle .ag-grip { width: 44px; height: 5px; border-radius: 999px; background: #97a2b2; opacity: .6; }
  #ag-sheet-head .map-search-ctrl { position: static !important; width: 100% !important; max-width: none !important; margin: 0 !important; box-shadow: none !important; background: transparent !important; }
  #ag-sheet-head .map-search-input {
    width: 100%; font-size: 16px !important; padding: 11px 14px; border-radius: 12px;
    border: 1px solid rgba(20,32,60,.12); background: #fff; color: #16203a; touch-action: auto;
  }
  html[data-theme="dark"] #ag-sheet-head .map-search-input { background: #232c42; border-color: rgba(255,255,255,.14); color: #e6ecff; }
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.68 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.68) — Feuille mobile « Apple Maps » : carte plein ecran +
   feuille solide draggable 3 crans + barre recherche = poignee qu'on tire +
   panneau calques (liste Leaflet reelle deplacee) au cran bas.
...
(function agAppleMapsSheet(){
  ...
  function pxFor(name){
    ...
    if (name==='peek') return Math.max(0, Math.round(H - 96 - TOP_FULL));
    return Math.max(0, Math.round(0.52*H - TOP_FULL));
  }
  ...
  function attachDrag(tp, el){
    ...
    el.addEventListener('touchstart', function(e){
      ...
      dragging=true; startY=e.touches[0].clientY; startTy=curTy(tp); tp.style.transition='none';
    }, {passive:true});
  ...
  function toggleLayers(){
    ...
    if (!p.querySelector('.leaflet-control-layers-list')){
      var list = document.querySelector('.leaflet-control-layers-list');
      if (list) p.appendChild(list);   /* on deplace la VRAIE liste (cases cablees) */
    }
  ...
  function enable(){
    try{
      ...
      if (!head){
        ...
        attachDrag(tp, head);   /* toute la barre tire (sauf le champ recherche) */
      }
      moveSearch(head);
  ...
  function disable(){
    try{
      document.body.classList.remove('ag-mobile-sheet','ag-sheet-peek','ag-sheet-mid','ag-sheet-full');
      var mc = document.getElementById('map-container');
      ...
      /* on remet la liste calques dans le controle Leaflet d'origine */
