/* ============================================================
   AutogyroDash — extensions v0.8.69
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.69');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.69';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.69</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.69) — MOBILE « Apple Maps » : carte plein ecran +
...
@media (max-width: 768px) {
  ...
  body.ag-mobile-sheet #tab-plan {
    ...
    isolation: isolate;
    background: var(--bg-0, #eef1f9) !important;
  ...
  body.ag-sheet-peek:not(.ag-dragging) #tab-plan {
    ...
    box-shadow: 0 8px 26px rgba(31,42,68,.32) !important;
  }
  body.ag-sheet-peek:not(.ag-dragging) #ag-sheet-head { margin-bottom: 0; box-shadow: none; border-radius: 22px; }
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.69 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.69) — Feuille mobile « Apple Maps » : carte plein ecran +
...
(function agAppleMapsSheet(){
  ...
  function buildLayersPanel(){
    ...
    var rows=0;
    if(lc && lc._layers && m){
      lc._layers.forEach(function(en){
        if(!en || !en.overlay || !en.layer) return;
        rows++;
    ...
    if(!rows){ /* repli : deplacer la vraie liste Leaflet si dispo */
