/* ============================================================
   AutogyroDash — extensions v0.8.64
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.64');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.64';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.64</div>
    `;
  }
...
/* ============================================================
   L1 (v0.8.64) — LAYOUT MOBILE « carte fixe haute + feuille a scroll interne »
   Ressemblance maquette Claude Design. PUR CSS, ISOLE @media <=768px.
   Desktop NON touche. Pas de drag (=L2).
   MODELE 2 ZONES (corrige l'overlap + carte inexploitable des v0.8.62/63) :
   - CARTE = zone FIXE en haut (0..46vh), TOUJOURS cliquable (rien par-dessus
     sauf la pilule de header). Zoom +/- masque sur mobile ; ⛶/recherche/layers
     conserves et repousses SOUS la pilule. La carte n'est JAMAIS recouverte.
   - FEUILLE = #tab-plan en panneau FIXE bas (46vh..bas) a SCROLL INTERNE : le
     contenu defile DANS la feuille, sans monter sur la carte. Pas de
     backdrop-filter sur #tab-plan -> la carte fixe (descendante) reste ancree
     au viewport. .map-fullscreen NON impacte.
   ============================================================ */
@media (max-width: 768px) {
  /* --- CARTE : zone fixe haute, cliquable --- */
  #tab-plan .band-overview > .card.no-glass > #map-container:not(.map-fullscreen) {
    position: fixed; top: 0; left: 0; right: 0;
    height: 46vh; min-height: 0; z-index: 1; border-radius: 0;
  }
  #tab-plan .band-overview > .card.no-glass > #map-container:not(.map-fullscreen) > #map {
    height: 100%; min-height: 0; border-radius: 0;
  }
  #tab-plan .band-overview > .card.no-glass {
    background: transparent !important; border: none !important;
    box-shadow: none !important; padding: 0 !important; min-height: 0;
  }
  /* note depliable + legende sous-carte masquees ; zoom +/- masque (place optimisee) */
  #tab-plan #map-container .map-about { display: none !important; }
  #tab-plan #map-controls { display: none !important; }
  #tab-plan #map-container:not(.map-fullscreen) .leaflet-control-zoom { display: none !important; }
  /* ⛶ + recherche + layers repousses SOUS la pilule de header (sinon masques) */
  #tab-plan #map-container:not(.map-fullscreen) .leaflet-top.leaflet-left  { top: 64px; }
  #tab-plan #map-container:not(.map-fullscreen) .leaflet-top.leaflet-right { top: 64px; }
  #tab-plan #map-container:not(.map-fullscreen) .leaflet-control-layers { max-height: 30vh; overflow-y: auto; }

  /* --- FEUILLE : panneau fixe bas a scroll interne --- */
  #tab-plan {
    position: fixed; top: 46vh; left: 0; right: 0; bottom: 0;
    z-index: 2;
    overflow-y: auto; -webkit-overflow-scrolling: touch;
    padding: 18px 12px calc(64px + env(safe-area-inset-bottom, 0px)) !important;
    background: var(--panel, rgba(255,255,255,.62));
    border-top: 1px solid var(--panel-bd, rgba(255,255,255,.8));
    border-radius: 26px 26px 0 0;
    box-shadow: 0 -16px 36px var(--shadow, rgba(31,42,68,.32));
  }
  /* poignee (collante en haut de la feuille) */
  #tab-plan::before {
    content: ""; display: block; position: sticky; top: 0;
    width: 40px; height: 4px; margin: 0 auto 14px;
    border-radius: 999px; background: var(--text-3, #97a2b2); opacity: .6;
  }
  /* titres de section OPAQUES / lisibles */
  #tab-plan .dash-section-title { color: var(--text, #16203a) !important; opacity: 1 !important; }
}
  `;
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.64 chargé', 'ok', 3000);
  }
