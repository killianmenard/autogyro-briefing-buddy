/* ============================================================
   AutogyroDash — extensions v0.8.73
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.73');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.73';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.73</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.73) — MOBILE « Apple Maps » : carte plein ecran +
   ...
   ============================================================ */
@media (max-width: 768px) {
  body.ag-mobile-sheet { overflow: hidden; }
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.73 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.73) — Feuille mobile « Apple Maps » : carte plein ecran +
...
(function agAppleMapsSheet(){
  ...
  function syncBg(tp){
    var hid = tp.classList.contains('hidden');     /* onglet brief actif ? sinon on masque carte + pilule + calques */
  ...
  function disable(){
    try{
      document.body.classList.remove('ag-mobile-sheet','ag-sheet-peek','ag-sheet-mid','ag-sheet-full','ag-dragging');
      var mc = document.getElementById('map-container');
