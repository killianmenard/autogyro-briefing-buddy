/* ============================================================
   AutogyroDash — extensions v0.6.34
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.6.34');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.6.34';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    return `
      ...
      <div class="text-xs text-muted text-center pt-2">AutogyroDash v0.6.34</div>
    `;
  }
  function setupResourcesNav() {
    ...
    const setActive = (sub) => {
      section.querySelectorAll('.res-subtab').forEach(b => {
        ...
        // v0.6.34 — pilules : texte blanc toujours, fond bleu (foncé si actif), gras si actif
  ...
  setInterval(() => {
    if (!document.getElementById('vfr-checks-wrapper-v065')) {
      ...
      // 🔥 v0.6.34 : appels makeNativeBlockCollapsible désactivés (cassaient l'affichage)
...
/* 🔥 v0.6.34 — Rangée météo : carte (gauche) 75% / lien metar-taf (droite) 25% */
#wf-row-weather {
...
/* 🔥 FIX #C v0.6.34 : sur la row Zones aériennes | Notes Pilote,
   ne pas étirer les blocs à la même hauteur — la liste a son propre scroll */
/* 🔥 v0.6.34 : Sur la row Zones aériennes | Notes Pilote,
...
/* 🔥 v0.6.34 : wrapper interne vfr-checks-wrapper-v065 doit avoir le MÊME gap
...
/* 🔥 v0.6.34 FIX STICKY/FIXED — Étoiles déplacées sur HTML::before (pas body)
...
/* 🔥 v0.6.34 : Le gap entre blocs est géré UNIQUEMENT par le gap flex
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.6.34 chargé', 'ok', 3000);
  }
