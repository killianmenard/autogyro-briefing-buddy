/* ============================================================
   AutogyroDash — extensions v1.0.30
   ------------------------------------------------------------
...
(async function() {
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v1.0.30</div>
    `;
  }
  ...
  (function setupMidStepVisibility() {
    ...
    // Quand "ajouter une étape" est cliqué, révéler le prochain slot non visible
    const addBtn = document.getElementById('add-step-btn');
    if (addBtn) {
      const origClick = addBtn.onclick;
      addBtn.addEventListener('click', function() {
        // Après le clic natif (qui affiche le slot desktop), marquer aussi ag-step-visible
        setTimeout(function() {
          [1, 2, 3].forEach(function(idx) {
            const slot = document.querySelector('[data-trip-slot="' + idx + '"]');
      ...
      }, true); // capture: true pour être avant tout autre handler
    }
...
@media (max-width: 768px) {
  ...
  /* Style Apple Maps au cran mid : marges latérales + coins arrondis tout autour + carte visible derrière */
  body.ag-sheet-mid #tab-plan {
    left: 8px !important;
    right: 8px !important;
    border-radius: 18px 18px 18px 18px !important;
    box-shadow: 0 0 0 0 transparent, 0 8px 32px rgba(31,42,68,.22) !important;
    overflow: hidden !important;   /* pas de scroll en mid : seul le bloc TRAJET est visible */
  }
  ...
  #ag-search-pill {
    display: none;
    position: fixed; left: 8px; right: 8px; z-index: 45; bottom: 120px;
    background: var(--header-pill-bg, #fff);
    border: 1.5px solid var(--header-pill-border, #e5e7eb);
    border-radius: 22px;
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v1.0.30 chargé', 'ok', 3000);
  }
