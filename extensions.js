/* ============================================================
   AutogyroDash — extensions v0.8.75
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.75');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.75';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.75</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.75) — MOBILE « Apple Maps » : carte plein ecran +
...
@media (max-width: 768px) {
  ...
  /* ---- CRAN PLEIN : petite barre FIXE au-dessus du header (toujours accessible pour replier la feuille sans scroller) ; recherche flottante effacee ---- */
  body.ag-sheet-full.ag-mobile-sheet header { padding-top: 34px !important; }
  body.ag-sheet-full #tab-plan { padding-top: 100px !important; }
  body.ag-sheet-full #ag-sheet-head {
    position: fixed !important;
    top: calc(env(safe-area-inset-top, 0px) + 6px) !important;
    left: 50% !important; transform: translateX(-50%) !important;
    margin: 0 !important; width: auto !important;
    padding: 4px 26px !important; z-index: 1001 !important;
  }
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.75 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.75) — Feuille mobile « Apple Maps » : carte plein ecran +
...
(function agAppleMapsSheet(){
  ...
  function attachDrag(tp, el){
    var startY=0, startX=0, startTy=0, dragging=false, pending=false;
    ...
    el.addEventListener('touchmove', function(e){
      ...
      if(!dragging){
        ...
        if(Math.abs(dy) < 6 || Math.abs(dy) <= Math.abs(dx)) return; /* seuil : glissement vertical franc = drag ; sinon on laisse le tap (focus recherche) */
