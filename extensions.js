/* ============================================================
   AutogyroDash — extensions v0.8.67
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.67');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.67';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.67</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.67) — MOBILE « Apple Maps » : carte plein ecran en FOND
   + feuille DRAGGABLE 3 crans (ANCREE PAR LE HAUT) + bouton calques
   bas-droite + recherche collee a la poignee + pilule PDF fixe (si trajet).
   ISOLE mobile (<=768px). Desktop NON touche.
   ============================================================ */
@media (max-width: 768px) {
  body.ag-mobile-sheet { overflow: hidden; }

  /* ---- CARTE plein ecran (deplacee par JS dans #mobile-map-bg) ---- */
  ...
  /* ---- CALQUES facon Apple Maps : controle natif en BAS-DROITE ---- */
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-layers:not(.leaflet-control-layers-expanded) .leaflet-control-layers-list { display: none !important; }
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-layers-toggle { display: none !important; }
  body.ag-mobile-sheet #mobile-map-bg .leaflet-bottom.leaflet-right { bottom: 186px; right: 12px; }
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-layers {
    border: none !important; background: transparent !important; box-shadow: none !important; margin: 0 !important;
  }
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-layers-expanded {
    display: block !important; max-height: 46vh; overflow-y: auto;
    border-radius: 14px !important; padding: 10px 12px !important; font-size: 14px;
    background: var(--card, #fff) !important; color: var(--text, #16203a);
    box-shadow: 0 6px 22px rgba(0,0,0,.28) !important;
  }
  #ag-layers-btn {
    position: fixed; right: 12px; bottom: 132px; z-index: 36;
  ...
  #ag-layers-btn svg { width: 24px; height: 24px; display: block; }
  body.ag-sheet-full #ag-layers-btn { display: none; }

  /* ---- FEUILLE draggable (ANCREE PAR LE HAUT pour fix scroll) ---- */
  body.ag-mobile-sheet #tab-plan {
    ...
    background: var(--panel, rgba(255,255,255,.82));
    -webkit-backdrop-filter: blur(22px) saturate(180%); backdrop-filter: blur(22px) saturate(180%);
    border-radius: 22px 22px 0 0;
    border-top: 1px solid var(--panel-bd, rgba(255,255,255,.85));
    box-shadow: 0 -16px 40px var(--shadow, rgba(31,42,68,.34));
    transition: transform .28s cubic-bezier(.4,0,.2,1);
    padding: 0 12px calc(86px + env(safe-area-inset-bottom, 0px));
  }

  /* ---- EN-TETE COLLANT (poignee + recherche) AU PREMIER PLAN ---- */
  #ag-sheet-head {
    position: sticky; top: 0; z-index: 40;
    margin: 0 -12px 10px; padding: 0 12px 8px;
    background: rgba(247,249,253,.98);
    -webkit-backdrop-filter: blur(16px) saturate(180%); backdrop-filter: blur(16px) saturate(180%);
    border-radius: 22px 22px 0 0;
    box-shadow: 0 10px 16px -12px var(--shadow, rgba(31,42,68,.4));
  }
  html[data-theme="dark"] #ag-sheet-head { background: rgba(20,26,40,.97); }
  #ag-sheet-handle { height: 28px; display: flex; align-items: center; justify-content: center; touch-action: none; cursor: grab; }
  #ag-sheet-handle .ag-grip { width: 44px; height: 5px; border-radius: 999px; background: var(--text-3, #97a2b2); opacity: .55; }
  ...
  #ag-sheet-head .map-search-input { width: 100%; font-size: 16px !important; padding: 11px 14px; border-radius: 12px; border: 1px solid var(--card-bd, rgba(20,32,60,.12)); background: var(--card, #fff); }
  ...
  /* ---- PILULE PDF/epingler : fixe en bas, au-dessus de la feuille ---- */
  /* visibilite trajet geree par le code existant ; masquee au cran bas */
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.67 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.67) — Feuille mobile « Apple Maps » : carte plein ecran +
   feuille draggable 3 crans (ancree HAUT) + bouton calques bas-droite +
   recherche collee a la poignee. Footer PDF NON deplace (pilule fixe CSS).
   ISOLE mobile. Desktop : rien ne s'execute.
   ============================================================ */
(function agAppleMapsSheet(){
  ...
  var TOP_FULL = 128;
  var current = 'mid';
  var bg=null, mapHome=null, searchHome=null, layersBtn=null;
  ...
  function attachDrag(tp, el){
    ...
    el.addEventListener('touchstart', function(e){ dragging=true; startY=e.touches[0].clientY; startTy=curTy(tp); tp.style.transition='none'; }, {passive:true});
  ...
  function makeLayersBtn(){
    var existing = document.getElementById('ag-layers-btn');
    if (existing){ layersBtn = existing; return; }
    ...
    b.style.color = 'var(--text, #0f1729)';
    b.addEventListener('click', function(e){
      e.preventDefault(); e.stopPropagation();
      var lc = (typeof layerControl !== 'undefined') ? layerControl : null;
      if(!lc) return;
      try {
        var cont = lc.getContainer ? lc.getContainer() : null;
        if (cont && cont.classList.contains('leaflet-control-layers-expanded')) lc.collapse();
        else lc.expand();
      } catch(err){}
    });
  ...
  function disable(){
    try{
      ...
      var b = document.getElementById('ag-layers-btn'); if (b) b.remove(); layersBtn=null;
