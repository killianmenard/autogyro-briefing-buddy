/* ============================================================
   AutogyroDash — extensions v1.0.33
   ------------------------------------------------------------
...
(async function() {
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v1.0.33</div>
    `;
  }
  ...
  // === Boutons ✕ clear sur les inputs départ (0) et arrivée (4) — injection JS pure, sans toucher au HTML ===
  (function setupClearBtns() {
    ...
    style.textContent = [
      ...
      '  color: var(--muted-foreground, #94a3b8); font-size: 14px; line-height: 1;',
      '  display: none; z-index: 2;',
      '}',
      '.ag-clear-btn:hover { color: #ef4444; }',
      '.ag-clear-btn.visible { display: block; }',
      '[data-trip-slot="0"] input, [data-trip-slot="4"] input { padding-right: 28px; }',
    ].join(' ');
    ...
    [0, 4].forEach(function(idx) {
      ...
      if (!slot || !inp) return;
      // Positionner le slot en relative s'il ne l'est pas déjà
      ...
      btn.addEventListener('click', function() {
        inp.value = '';
        STATE.trip[idx] = null;
        const sugg = document.getElementById('ad-suggestions-' + idx);
        if (sugg) sugg.classList.add('hidden');
        btn.classList.remove('visible');
        if (typeof onTripChange === 'function') onTripChange();
      });
      // Vérifier périodiquement (pour les restaurations depuis l'historique)
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v1.0.33 chargé', 'ok', 3000);
  }
