/* ============================================================
   AutogyroDash — extensions v0.8.66
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.66');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.66';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.66</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.66) — MOBILE « Apple Maps » : carte plein ecran en FOND
   + feuille DRAGGABLE 3 crans + bouton calques + recherche en tete collante.
...
@media (max-width: 768px) {
  ...
  /* BOUTON CALQUES facon Apple Maps (haut-droite, sous la pilule) */
  body.ag-mobile-sheet #mobile-map-bg .leaflet-top.leaflet-right { top: 64px; right: 10px; }
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-layers { border: none !important; background: transparent !important; box-shadow: none !important; }
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-layers-toggle {
    width: 46px !important; height: 46px !important; border-radius: 13px !important;
    background-color: var(--card, #fff) !important;
    background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%230f1729' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><polygon points='12 2 2 7 12 12 22 7 12 2'/><polyline points='2 17 12 22 22 17'/><polyline points='2 12 12 17 22 12'/></svg>") !important;
  ...
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-layers-expanded {
    max-height: 58vh; overflow-y: auto; border-radius: 13px !important; padding: 8px 10px !important;
    box-shadow: 0 4px 16px rgba(0,0,0,.25) !important; background: var(--card, #fff) !important;
  }
  /* la carte d'origine videe ne s'affiche plus dans la feuille */
  body.ag-mobile-sheet .band-overview > .card.no-glass { display: none !important; }

  /* FEUILLE draggable */
  body.ag-mobile-sheet #tab-plan {
    position: fixed; left: 0; right: 0; bottom: 0; top: auto;
    height: 92vh; z-index: 30;
  ...
  #ag-sheet-head {
    ...
    border-radius: 22px 22px 0 0;
    box-shadow: 0 8px 14px -12px var(--shadow, rgba(31,42,68,.35));
  }
  html[data-theme="dark"] #ag-sheet-head { background: rgba(20,26,40,.95); }
  #ag-sheet-handle { height: 30px; display: flex; align-items: center; justify-content: center; touch-action: none; cursor: grab; }
  #ag-sheet-handle .ag-grip { width: 44px; height: 5px; border-radius: 999px; background: var(--text-3, #97a2b2); opacity: .55; }
  /* recherche carte deplacee dans l'en-tete, pleine largeur */
  #ag-sheet-head .map-search-ctrl { position: static !important; width: 100% !important; max-width: none !important; margin: 0 0 10px !important; box-shadow: none !important; background: transparent !important; }
  #ag-sheet-head .map-search-input { width: 100%; font-size: 16px !important; padding: 10px 14px; border-radius: 12px; border: 1px solid var(--card-bd, rgba(20,32,60,.12)); background: var(--card, #fff); }
  ...
  /* footer (bouton PDF) deplace dans la feuille -> flux normal */
  body.ag-mobile-sheet #tab-plan > footer {
    position: static !important; z-index: auto !important; background: transparent !important;
    border: none !important; -webkit-backdrop-filter: none !important; backdrop-filter: none !important;
    padding: 6px 0 0 !important; margin-top: 8px;
  }
}
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.66 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.66) — Feuille mobile « Apple Maps » : carte plein ecran +
   feuille draggable 3 crans + recherche/PDF deplaces dans la feuille.
...
(function agAppleMapsSheet(){
  ...
  var current = 'mid';
  var bg=null, mapHome=null, searchHome=null, footerHome=null;
  ...
  function pxFor(name){ var H=ih();
    if (name==='peek') return Math.round(0.92*H - 40);  /* visible ~40px : poignee seule */
    if (name==='full') return Math.max(44, Math.round(132 - 0.08*H)); /* haut fige ~132px : degage la pilule header */
    return Math.round(0.42*H);                          /* mid : ~50% */
  }
  ...
  function enable(){
    try{
      ...
      if (footer && footer.parentElement !== tp){ if(!footerHome) footerHome = footer.parentElement; tp.appendChild(footer); }
  ...
  function disable(){
    try{
      document.body.classList.remove('ag-mobile-sheet');
      var mc = document.getElementById('map-container');
      ...
      if (footerHome && footer && footer.parentElement !== footerHome) footerHome.appendChild(footer);
      var tp = document.getElementById('tab-plan');
