/* ============================================================
   AutogyroDash — extensions v0.8.74
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.74');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.74';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.74</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.74) — MOBILE « Apple Maps » : carte plein ecran +
...
@media (max-width: 768px) {
  ...
  /* ---- CRAN PLEIN : la poignee se place ENTRE le header de l'app et le haut du contenu (statique, pas sticky -> ne flotte plus sur le texte) ; recherche flottante effacee ---- */
  body.ag-sheet-full #tab-plan { padding-top: 78px !important; }
  body.ag-sheet-full #ag-sheet-head { position: static !important; top: auto !important; }
  ...
  #ag-search-pill .map-search-ctrl { position: static !important; width: 100% !important; max-width: none !important; margin: 0 !important; box-shadow: none !important; background: transparent !important; touch-action: auto; }
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.74 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.74) — Feuille mobile « Apple Maps » : carte plein ecran +
...
(function agAppleMapsSheet(){
  ...
  function attachDrag(tp, el){
    var startY=0, startTy=0, dragging=false;
    el.addEventListener('touchstart', function(e){
      if (e.target && e.target.closest && e.target.closest('.map-search-ctrl')) return; /* tap recherche = pas de drag */
