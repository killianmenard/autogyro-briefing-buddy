/* ============================================================
   AutogyroDash — extensions v0.8.70
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.70');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.70';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.70</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.70) — MOBILE « Apple Maps » : carte plein ecran +
...
@media (max-width: 768px) {
  ...
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-attribution { display: none !important; }
  body.ag-sheet-peek #mobile-map-bg .leaflet-control-attribution {
    display: block !important; position: fixed !important;
    right: 6px !important; left: auto !important; top: auto !important;
  ...
  /* cartes internes : SANS verre, fond solide -> aucune carte au travers */
  body.ag-mobile-sheet #tab-plan .card:not(.no-glass),
  body.ag-mobile-sheet #tab-plan .dash-band {
    backdrop-filter: none !important; -webkit-backdrop-filter: none !important;
  }
  body.ag-mobile-sheet #tab-plan .card:not(.no-glass) {
    background: var(--bg-1, #fff) !important;
    border: 1px solid var(--card-bd, rgba(20,32,60,.10)) !important;
  }

  /* ---- CRAN PLEIN : pleine page, carte cachee, poignee sous la pilule header ---- */
  body.ag-sheet-full #ag-sheet-head { padding-top: 76px; }

  /* ---- CRAN BAS : PILULE flottante (coins arrondis + marges) ---- */
  body.ag-sheet-peek:not(.ag-dragging) #tab-plan {
    ...
    /* v0.8.70 — esthetique pilule header : blanc plein, bordure fine, ombre douce */
    background: var(--header-pill-bg, #fff) !important;
    border: 1.5px solid var(--header-pill-border, #e5e7eb) !important;
    border-radius: 26px !important;
    transform: none !important;
    overflow: hidden !important;
    box-shadow: 0 6px 20px rgba(31,42,68,.16) !important;
  }
  body.ag-sheet-peek:not(.ag-dragging) #ag-sheet-head { margin-bottom: 0; box-shadow: none; border-radius: 26px; background: transparent; }

  /* ---- EN-TETE = POIGNEE + RECHERCHE (un seul bandeau qu'on tire) ---- */
  #ag-sheet-head {
    position: sticky; top: 0; z-index: 50;
    margin: 0 -12px 12px; padding: 7px 12px 10px;
    background: var(--bg-0, #eef1f9);
    border-radius: 22px 22px 0 0;
    box-shadow: 0 8px 10px -8px rgba(31,42,68,.26);
    touch-action: none;
  }
  ...
  #ag-sheet-head .map-search-ctrl { position: static !important; width: 100% !important; max-width: none !important; margin: 4px 0 0 !important; box-shadow: none !important; background: transparent !important; }
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.70 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.70) — Feuille mobile « Apple Maps » : carte plein ecran +
