/* ============================================================
   AutogyroDash — extensions v0.8.63
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.63');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.63';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.63</div>
    `;
  }
...
/* ============================================================
   L1 (v0.8.63) — LAYOUT MOBILE « carte en fond + feuille statique »
   Ressemblance maquette Claude Design. PUR CSS, ISOLE @media <=768px.
   Desktop NON touche (PDF/plein ecran/gating inchanges). Pas de drag (=L2).
   v0.8.63 : la carte passe DERRIERE la feuille (z-index:-1) — corrige l'overlap
   (la carte ne recouvre plus les sections ni la 2e carte meteo). Astuce
   pointer-events : #tab-plan laisse passer les clics vers la carte dans la
   bande VISIBLE du haut ; le contenu (cartes/formulaire) reste cliquable et
   defile PAR-DESSUS la carte. Controle de calques (POI/RTBA/zones/AD/ULM)
   decale sous la pilule de header (sinon masque). .map-fullscreen NON impacte.
   ============================================================ */
@media (max-width: 768px) {
  /* carte = CALQUE DE FOND, derriere la feuille, interactive dans la bande du haut */
  ...
  /* note depliable + legende sous-carte masquees (carte nette) ;
     le CONTROLE DE CALQUES Leaflet reste, decale sous le header */
  #tab-plan #map-container .map-about { display: none !important; }
  #tab-plan #map-controls { display: none !important; }
  #tab-plan #map-container:not(.map-fullscreen) .leaflet-top.leaflet-right { top: 70px; }
  #tab-plan #map-container:not(.map-fullscreen) .leaflet-control-layers {
    max-height: 30vh; overflow-y: auto;
  }

  /* FEUILLE : zone vide du haut = clics traversent vers la carte ; contenu cliquable + au-dessus */
  #tab-plan {
    position: relative;
    min-height: 100vh;
    padding: calc(46vh + 16px) 12px 40px !important;
    pointer-events: none;
  }
  #tab-plan > * { pointer-events: auto; }
  #tab-plan::before {
    content: ""; position: absolute; left: 0; right: 0;
    top: 46vh; bottom: 0;
    background: var(--panel, rgba(255,255,255,.55));
    border-top: 1px solid var(--panel-bd, rgba(255,255,255,.75));
    border-radius: 26px 26px 0 0;
    box-shadow: 0 -14px 34px var(--shadow, rgba(31,42,68,.3));
    pointer-events: none;
  }
  #tab-plan::after {
    content: ""; position: absolute; top: calc(46vh + 9px); left: 50%;
    transform: translateX(-50%); width: 40px; height: 4px;
    border-radius: 999px; background: var(--text-3, #97a2b2);
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.63 chargé', 'ok', 3000);
  }
