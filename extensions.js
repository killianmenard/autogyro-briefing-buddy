/* ============================================================
   AutogyroDash — extensions v0.6.27
   ------------------------------------------------------------
   Nouveau dans v0.6.27 (hotfix v0.6.5 — 4 correctifs ciblés) :
...
(async function() {
  ...
  console.log('[Extensions v0.6.27] Boot...');
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.6.27');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.6.27';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    return `
      ...
      <div class="text-xs text-muted text-center pt-2">AutogyroDash v0.6.27</div>
    `;
  }
  ...
  function setupSatelliteToggleV065() {
    ...
    function tryInit() {
      ...
      console.log('[Satellite v0.6.27] Toggle inséré en première position ✓');

      let satOn = false;
      // 🔥 v0.6.27 : tracker explicitement l'état du satellite natif
      ...
      function applyState() {
        ...
        if (satOn) {
          ...
          // 🔥 v0.6.27 : DÉSACTIVER explicitement le satellite natif
          if (nativeSatActive) clickSatelliteNative();
  ...
  function reorderBriefDOM() {
    ...
    // Ordre final souhaité (v0.6.27 — AZBA/NOTAM passe après zones aériennes)
    //   1. Trajet
    ...
    // 🔥 v0.6.27 : masquer wf-row-azba-notam tant que pas de trajet validé
    ...
    // 🔥 v0.6.27 : DÉSACTIVÉ — makeNativeBlockCollapsible cassait l'affichage
    ...
    // 🔥 FIX #A v0.6.27 : Fusion overlays-carte + map-container en "Carte des aérodromes"
    mergeMapBlocksIntoOneCard();

    // 🔥 FIX #B v0.6.27 : Harmoniser les chevrons des <details> natifs
  ...
  // ============================================================
  // 🔥 FIX #A v0.6.27 — FUSION overlays-carte + map-container
  ...
  function mergeMapBlocksIntoOneCard() {
    ...
    console.log('[v0.6.27] Carte aérodromes fusionnée ✓');
  }

  // ============================================================
  // 🔥 FIX #B v0.6.27 — HARMONISATION DES CHEVRONS NATIFS
  ...
  function harmonizeDetailsChevrons() {
    document.querySelectorAll('details:not([data-chevron-harmonized])').forEach(det => {
      ...
      // 🔥 FIX v0.6.27 : skip les sous-<details> imbriqués pour ne pas
  ...
  function makeNativeBlockCollapsible(el, key, _label) {
    ...
    // 🔥 v0.6.27 : si `el` contient une seule .card enfant direct,
  ...
  setInterval(() => {
    if (!document.getElementById('vfr-checks-wrapper-v065')) {
      ...
      // 🔥 v0.6.27 : appels makeNativeBlockCollapsible désactivés (cassaient l'affichage)
  ...
  // 🔥 v0.6.27 : VISIBILITÉ DÉFENSIVE des blocs zones aériennes + AZBA/NOTAM
  // ----------------------------------------------------------------------
  ...
  // 🔥 v0.6.27 — DIAGNOSTIC console (à invoquer manuellement)
  ...
  window.__diagBriefing = function() {
    ...
    console.log('=== DIAG v0.6.27 ===', report);
    return report;
  };

  // 🔥 v0.6.27 — DIAGNOSTIC SPACING (mesure les vrais gaps visuels)
  ...
  // 🔥 v0.6.27 — HIDE EMPTY FLEX CHILDREN
  // ----------------------------------------------------------------
  ...
  function v0622HideEmptyFlexChildren() {
    ...
    Array.from(tabPlan.children).forEach(c => {
      ...
      if (isReallyEmpty) {
        ...
        c.style.setProperty('display', 'none', 'important');
        console.log(`[v0.6.27] Wrapper vide caché (gap fantôme évité) : ${c.id || c.tagName.toLowerCase() + '.' + c.className.slice(0, 30)}`);
      }
    });
  }
  // Boot + periodic
  ...
  // ============================================================
  // 🔥 v0.6.27 — FOOTER TEXTE BASCULÉ SOUS BLOC HISTORIQUE
  // ----------------------------------------------------------
  ...
  function v0618HandleHistoriqueFooter() {
    // 🔥 v0.6.27 — APPROCHE SIMPLE : cacher TOUJOURS et PARTOUT le texte
  ...
  // 🔥 v0.6.27 — MutationObserver pour cacher INSTANTANÉMENT le texte natif
...
/* 🔥 FIX #C v0.6.27 : sur la row Zones aériennes | Notes Pilote,
   ne pas étirer les blocs à la même hauteur — la liste a son propre scroll */
/* 🔥 v0.6.27 : Sur la row Zones aériennes | Notes Pilote,
...
/* 🔥 v0.6.27 : wrapper interne vfr-checks-wrapper-v065 doit avoir le MÊME gap
...
/* Étoiles SVG inline (~29 étoiles, tailles et opacités variées) en arrière-plan FIXE */
...
/* 🔥 v0.6.27 FIX BARRE NOIRE — header et footer FORCÉS transparents en mode nuit
   (la barre noire qu'on voit c'est le bg natif <header class="bg-..."> qui
    n'est pas attrapé par mon CSS générique. On force avec specificité max). */
html.dark body > header,
...
/* ============================================================
   🔥 v0.6.27 — CHARTE GRAPHIQUE (couleurs liens, boutons, etc.)
   ============================================================ */
...
/* ============================================================
   🔥 v0.6.27 — BOUTON "GÉNÉRER LE PDF" EN PILULE
   Mode jour : bleu ciel #4DC2F1
...
/* === 🔥 v0.6.27 — RETIRE TOUS LES TOGGLES DES BLOCS DYNAMIQUES === */
...
/* v0.6.27 : NE PAS cacher les <summary> car ils peuvent contenir le titre
...
/* === 🔥 FIX #C v0.6.27 — Zones aériennes scroll interne ===
...
/* === Carte aérodromes fusionnée (v0.6.27) ===
...
.native-collapsible-content {
  ...
  // ============================================================
  // 🔥 v0.6.27 — FIX METAR : cleanup cache pollué + timeout wrapper
  ...
  (function cleanupMetarCacheV0612() {
    try {
      ...
      if (fixed > 0 || removed > 0) {
        console.log(`[v0.6.27] METAR cache cleanup : ${fixed} repaired, ${removed} removed ✓`);
      }
    } catch (e) {}
  })();
  ...
  (function patchMetarTimeoutV0612() {
    function _tryPatch() {
      ...
      console.log('[METAR v0.6.27] timeout-wrapped (12s max) ✓');
    }
    _tryPatch();
  })();
  ...
  (function patchFetchMetar() {
    function _tryPatch() {
      ...
      console.log('[METAR v0.6.27] fetchMetar patché : Promise.any + 5s + stale-while-revalidate ✓');
    }
    _tryPatch();
  })();
  */
  ...
  // ============================================================
  // 🌤️ FOND CIEL + NUAGES v0.6.27 (mode jour uniquement)
...
/* 🔥 v0.6.27 : FORCE TRANSPARENCE ULTRA-AGRESSIVE sur tous les conteneurs
...
/* 🔥 v0.6.27 : Si un wrapper invisible enveloppe le contenu et a un fond
...
/* 🔥 v0.6.27 : Le gap entre blocs est géré UNIQUEMENT par le gap flex
...
/* 🔥 v0.6.27 : Header pilule SANS flou, MAIS sans rectangle blanc moche.
...
/* 🔥 v0.6.27 : footer (Sources / Données indicatives) - style ciblé
...
html:not(.dark) .v0610-footer-pill {
  ...
  // ============================================================
  // 🔥 v0.6.27 — FIX #1 : Légende météo France décalée à droite
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  // 🔥 v0.6.27 — FIX #2 : RECONSTRUCTION RADICALE des sections
  ...
  function rebuildAirspacesSectionV0610() {
    ...
    console.log('[v0.6.27] airspaces-section rebuild ✓');
  }
  ...
  function rebuildTripSummaryV0610() {
    ...
    console.log('[v0.6.27] trip-summary rebuild ✓');
  }

  // 🔥 v0.6.27 : APPROCHE NUCLÉAIRE pour le doublon de titre.
  ...
  function v0613FlattenAndChevron(section, key, iconLucide, titleRegex) {
    ...
    // 🔥 v0.6.27 : TRANSMUTATION RADICALE.
    ...
    section.querySelectorAll('details').forEach(det => {
      ...
      if (sum) {
        console.log(`[v0.6.27] Removed <summary> doublon dans #${section.id}`);
        sum.remove();
      }
      ...
      console.log(`[v0.6.27] Transmuté <details> en <div> dans #${section.id} ✓`);
    });
    ...
    Array.from(section.querySelectorAll('h2, h3, span.section-title')).forEach(h => {
      ...
      if (titleRegex.test(txt) && !hasIcon) {
        ...
        if (toRemove.parentElement && toRemove !== nativeCard) {
          toRemove.remove();
          console.log(`[v0.6.27] Suppression doublon titre sans icon dans #${section.id}`);
        }
      }
    });
    ...
    // 🔥 v0.6.27 — CLEANUP IDEMPOTENT (toujours, AVANT le check v0614Done)
    // ----------------------------------------------------------------
    ...
    if (nativeCard.dataset.v0614Done === '1') {
      // 🔥 v0.6.27 : Check basé sur le chev absolute (v0623-abs)
    ...
    // 🔥 v0.6.27 BUG FIX CRITIQUE : TOUJOURS ajouter la classe .v0614-header
    // au header CHOISI (même si on a réutilisé une flex row du natif).
    header.classList.add('v0614-header');

    // 🔥 v0.6.27 — CHEV EN ABSOLUTE POSITIONING (garanti top-right)
    // ----------------------------------------------------------------
    ...
    // 🔥 v0.6.27 : Cleanup ciblé UNIQUEMENT sur triangles chevron
    ...
    nativeCard.querySelectorAll('.unified-chevron, .block-chev').forEach(c => {
      if (c !== chev) {
        c.remove();
        console.log(`[v0.6.27] Chevron .unified-chevron/.block-chev parasite retiré dans #${section.id}`);
      }
    });
    ...
    nativeCard.querySelectorAll('*').forEach(el => {
      ...
      if (/^[▼▾▽▿⌃⌄⏷⏶▲▴△▵⏵⏴▶◀▸◂➤➡˅˄﹀⮟⮝]$/.test(txt)) {
        console.log(`[v0.6.27] Élément <${el.tagName.toLowerCase()}> "${txt}" retiré dans #${section.id}`);
        el.remove();
      }
    });
    ...
    console.log(`[v0.6.27] Chevron absolute ajouté à #${section.id} ✓`);
  }

  function v0613NuclearFixAll() {
    // 🔥 v0.6.27 — DÉSACTIVÉ : Killian veut plus AUCUN toggle sur les blocs
  ...
  // 🔥 v0.6.27 : Le setInterval reste mais la fonction est no-op
      ...
      Array.from(block.querySelectorAll('details')).forEach(det => {
        ...
        while (det.firstChild) {
          if (det.firstChild.tagName === 'SUMMARY') {
            // 🔥 v0.6.27 : EXTRAIRE le contenu du summary (le titre)
  ...
  // ============================================================
  // 🔥 v0.6.27 — TITRE "notes pilote" stylisé comme "CARTE DES AÉRODROMES"
  ...
  function v0626StyleNotesPiloteTitle() {
    ...
    // 🔥 v0.6.27 : Forcer padding 14px 16px sur la card (match CARTE DES AÉRODROMES)
  ...
  // ============================================================
  // 🔥 v0.6.27 — FIX #3 : PRÉSERVATION DU SCROLL dans #airspaces-list
  ...
  function setupAirspacesScrollPreservation() {
    ...
    console.log('[v0.6.27] airspaces-list scroll preservation ✓');
  }
  setTimeout(setupAirspacesScrollPreservation, 800);
  setTimeout(setupAirspacesScrollPreservation, 2500);

  // ============================================================
  // 🔥 v0.6.27 — FIX BLOC BLANC : déballer le wrapper .v0610-footer-pill
  // ----------------------------------------------------------------
  ...
  function v0616UnwrapBrokenFooterPill() {
    document.querySelectorAll('.v0610-footer-pill').forEach(pill => {
      ...
      pill.remove();
      console.log('[v0.6.27] Déballé .v0610-footer-pill erroné ✓');
    });
  }
  ...
  function wrapFooterTextsInPill() { /* no-op v0.6.27 */ }

  // ============================================================
  // 🔥 v0.6.27 — FIX #8 : ANIMATIONS AU CHANGEMENT DE TAB
  // Mini overlay avion qui glisse de bas-gauche en diagonale + 
  // fade-slide-in du contenu du tab. Style "Apple smooth".
  // ============================================================
  // ============================================================
  // 🔥 v0.6.27 — ANIMATIONS DÉSACTIVÉES
  ...
  function showPlaneOverlay() { /* no-op v0.6.27 */ }
  function setupTabAnimationsV0611() { /* no-op v0.6.27 */ return true; }

  // ============================================================
  // 🔥 v0.6.27 — FIX #9 : Étendre le filtre harmonizeDetailsChevrons
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.6.27 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.27] Intégration terminée');
})();
