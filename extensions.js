/* ============================================================
   AutogyroDash — extensions v0.8.15
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.15');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.15';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    return `
      ...
      <div class="text-xs text-muted text-center pt-2">AutogyroDash v0.8.15</div>
    `;
  }
...
/* ============================================================
   ...
   ============================================================ */
/* B2b-3 (v0.8.14) — bouton PDF (hero CTA) en VERRE a l'image de la pilule header :
   fond --panel + blur + inset highlight, MAIS bordure + texte accent (pour rester lisible
   sur le footer blanc bg-white/95 et signaler le CTA). Jour/nuit via tokens. */
#pdf-btn {
  border-radius: 9999px !important;
  font-weight: 600 !important;
  background: var(--panel) !important;
  ...
  border: 1px solid var(--accent-bd) !important;
  color: var(--accent) !important;
  box-shadow: inset 0 1px 0 var(--hi), 0 8px 24px var(--shadow) !important;
...
#pdf-btn:hover {
  ...
  box-shadow: inset 0 1px 0 var(--hi), 0 0 18px var(--accent-soft), 0 10px 30px var(--shadow) !important;
}
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.15 chargé', 'ok', 3000);
  }
