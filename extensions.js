/* ============================================================
   AutogyroDash — extensions v0.8.22
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.22');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.22';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    return `
      ...
      <div class="text-xs text-muted text-center pt-2">AutogyroDash v0.8.22</div>
    `;
  }
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.22 chargé', 'ok', 3000);
  }
