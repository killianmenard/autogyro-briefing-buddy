/* ============================================================
   AutogyroDash — extensions v1.0.1
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v1.0.1');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v1.0.1';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v1.0.1</div>
    `;
  }
  ...
  // ============================================================
  // MON COMPTE (Lot 1 — coquille locale, 0 backend)
  // ============================================================
  (function setupAccount(){
    ...
    function refreshAccountState() {
      ...
      if (nameInp) nameInp.value = localStorage.getItem(PROFILE_KEY) || '';
    ...
    // Profil : sauvegarde au fil de la frappe
    document.body.addEventListener('input', e => {
      if (e.target && e.target.id === 'acc-profile-name') {
        try { localStorage.setItem(PROFILE_KEY, e.target.value); } catch(_){}
      }
    });
...
/* ============================================================
   L1+L2 (v1.0.1) — MOBILE « Apple Maps » : carte plein ecran +
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v1.0.1 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v1.0.1) — Feuille mobile « Apple Maps » : carte plein ecran +
