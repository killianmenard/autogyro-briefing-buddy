/* ============================================================
   AutogyroDash — extensions v0.8.65
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.65');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.65';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.65</div>
    `;
  }
...
/* ============================================================
   L1+L2 (v0.8.65) — LAYOUT MOBILE « Apple Maps » :
   carte plein ecran en FOND + feuille DRAGGABLE a 3 crans (peek/mid/full)
   + bouton calques. ISOLE mobile (<=768px) ; desktop NON touche.
   La carte est DEPLACEE par JS hors de #tab-plan vers #mobile-map-bg
   (corrige le bug iOS "fixed dans conteneur scrollable" de v0.8.64).
   ============================================================ */
@media (max-width: 768px) {
  body.ag-mobile-sheet { overflow: hidden; }

  /* calque carte plein ecran (rempli par le JS) */
  ...
  /* zoom +/- et ⛶ masques (le drag de la feuille remplace) ; recherche + calques gardes */
  ...
  body.ag-mobile-sheet #mobile-map-bg .leaflet-top.leaflet-left  { top: 64px; }
  body.ag-mobile-sheet #mobile-map-bg .leaflet-top.leaflet-right { top: 64px; }
  body.ag-mobile-sheet #mobile-map-bg .leaflet-control-layers { max-height: 60vh; overflow-y: auto; }
  /* la carte d'origine (videe apres deplacement) ne s'affiche plus dans la feuille */
  body.ag-mobile-sheet .band-overview > .card.no-glass { display: none !important; }

  /* FEUILLE draggable (translateY pilote par le JS) */
  body.ag-mobile-sheet #tab-plan {
    position: fixed; left: 0; right: 0; bottom: 0; top: auto;
    height: 94vh; z-index: 5;
  ...
  /* poignee de drag (collante en haut de la feuille) */
  #ag-sheet-handle {
    position: sticky; top: 0; z-index: 3;
    margin: 0 -12px 8px; padding: 12px 0 9px;
  ...
  /* titres de section opaques/lisibles */
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.65 chargé', 'ok', 3000);
  }
  console.log('[Extensions v0.6.34] Intégration terminée');
})();

/* ============================================================
   L1+L2 (v0.8.65) — Feuille mobile « Apple Maps » draggable + carte plein ecran.
   ISOLE mobile (matchMedia <=768px). Desktop : rien ne s'execute.
   ============================================================ */
(function agAppleMapsSheet(){
  if (!window.matchMedia) return;
  var MQ = window.matchMedia('(max-width: 768px)');
  var DET = { peek: 0.82, mid: 0.45, full: 0.05 };
  var current = 'mid';
  var bg = null, mapHome = null, handle = null;
  ...
  function pxFor(name){ return Math.round(ih() * DET[name]); }
  ...
  function attachDrag(tp, el){
    ...
    function end(){
      ...
      var ty = curTy(tp), best='mid', bd=Infinity;
      for (var k in DET){ var d=Math.abs(pxFor(k)-ty); if(d<bd){bd=d;best=k;} }
  ...
  function enable(){
    try{
      ...
      if (!document.getElementById('ag-sheet-handle')){
        handle = document.createElement('div'); handle.id='ag-sheet-handle';
        handle.innerHTML = '<span class="ag-grip"></span>';
        tp.insertBefore(handle, tp.firstChild);
        attachDrag(tp, handle);
      }
      if (!tp.__agMo){
        tp.__agMo = new MutationObserver(function(){ syncBg(tp); });
        tp.__agMo.observe(tp, { attributes:true, attributeFilter:['class'] });
      }
  ...
  function disable(){
    try{
      ...
      var h = document.getElementById('ag-sheet-handle'); if (h){ h.remove(); handle=null; }
