/* ============================================================
   AutogyroDash — extensions v0.8.9
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.9');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.9';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    return `
      ...
      <div class="text-xs text-muted text-center pt-2">AutogyroDash v0.8.9</div>
    `;
  }
...
/* v0.8.9 — l'en-tête « logistique » des fiches AD est un <summary> transmuté en
   <div.v0625-ex-summary> par v0624RemoveAllDynamicToggles, ce qui PERD le px-4 du
   summary → l'icône carburant débordait à gauche (0px) vs le contenu (16px).
   On restaure 1rem de retrait UNIQUEMENT sur ce summary (enfant direct de la fiche),
   sans toucher le summary « TAF » imbriqué dans la section METAR (déjà à 16px via la grille). */
[data-ad-card] > div > .v0625-ex-summary { padding-left: 1rem; }
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.9 chargé', 'ok', 3000);
  }
