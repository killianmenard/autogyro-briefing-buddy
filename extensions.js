/* ============================================================
   AutogyroDash — extensions v0.8.62
   ------------------------------------------------------------
...
(async function() {
  ...
  try {
    document.title = document.title.replace(/v0\.\d+\.\d+/, 'v0.8.62');
    document.querySelectorAll('span.text-xs.pre-mono').forEach(s => {
      if (/^v0\.\d+\.\d+$/.test(s.textContent.trim())) s.textContent = 'v0.8.62';
    });
  } catch (e) {}
  ...
  function buildSourcesContentHtml() {
    ...
    return `
      ...
      <div class="text-xs text-muted text-center pt-4 mt-2 border-t border-thin">AutogyroDash v0.8.62</div>
    `;
  }
...
/* ============================================================
   L1 (v0.8.62) — LAYOUT MOBILE « carte en fond + feuille statique »
   Ressemblance maquette Claude Design (carte plein ecran + feuille de verre iOS).
   PUR CSS, ISOLE en @media <=768px : le DESKTOP n'est PAS touche (PDF, plein ecran,
   gating desktop-first inchanges). Pas de drag (= Lot L2 a venir).
   Principe :
   - #map-container -> BANDEAU FIXE en haut, INTERACTIF (z-index 1, sous le header z1000).
     Le ResizeObserver existant (#map) rappelle invalidateSize() tout seul.
   - #tab-plan -> la FEUILLE : contenu pousse sous la carte, panneau translucide (::before)
     + poignee (::after). z-index:0 sur #tab-plan = contexte d'empilement propre ; mais
     AUCUN backdrop-filter/transform/filter sur #tab-plan -> le #map-container fixe reste
     ancre au VIEWPORT (jamais decroche). Les cartes internes gardent leur verre.
   - .map-fullscreen NON impacte (selecteurs :not(.map-fullscreen)).
   ============================================================ */
@media (max-width: 768px) {
  ...
  /* legende sous-carte + note depliable masquees sur mobile (carte nette facon maquette) */
  #tab-plan #map-controls,
  #tab-plan #map-container .map-about { display: none !important; }

  /* La feuille : #tab-plan contexte d'empilement (z-index:0) SANS filtre -> carte fixe non piegee */
  #tab-plan {
    position: relative; z-index: 0;
    min-height: 100vh;
    padding: calc(46vh + 16px) 12px 0 !important;
  }
  #tab-plan::before {
...
#weather-france-section .wf-mode-line {
  ...
  // ============================================================
  if (typeof showToast === 'function') {
    showToast('✓ v0.8.62 chargé', 'ok', 3000);
  }
